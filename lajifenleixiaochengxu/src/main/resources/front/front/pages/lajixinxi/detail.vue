...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
			...
			<!-- 名字-->
						<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.lajixinxiName}}
						</view>
				</view>
   	 <view style="margin-top: 10rpx;"></view>
					...
					<view class="detail-list-item"
						...
						<view class="lable"
							...
							浏览次数：</view>
						<view class="text" :style='{"color":"#008761","textAlign":"center","fontSize":"24rpx"}'>
							{{detail.lajixinxiClickNumber}}</view>
					</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.lajixinxiContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('lajixinxi', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.lajixinxiPhoto ? this.detail.lajixinxiPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('lajixinxiLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					lajixinxiId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../lajixinxiLiuyan/add-or-update?lajixinxiId=${this.id}`)
            },
		}
	}
</script>
